...
/** LOCKED MVP — registration must collect gender for hostel matching. Do not remove.
...
export function AuthPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!gender) { setError('Please select your gender (used only for hostel matching).'); return; }
        ...
        >
          <div className="rounded-2xl p-8" style={{ background: '#0B1120', border: '1px solid #1E2D45' }}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* LOCKED MVP: gender — matching only, never shown in public UI */}
              <div>
                <label className="text-xs mb-1.5 block" style={{ color: '#94A3B8' }}>
                  Gender
                </label>
                ...
                <p className="text-[10px] mt-1.5" style={{ color: '#475569' }}>
                  Used only for men's/women's hostel job matching. Never shown on your profile.
                </p>
              </div>
